...
#        return ((self.distance_avec_balle() * self.state.ball.vitesse + self.state.ball.position) - self.state.player_state(self.id_team, self.id_player).position).normalize().scale(5)
    ...
    def joueur_proche(self, id_team, id_player):
        ...
        try:
            a=min([(self.player.distance(player_p.position), player_p) for player_p in opponents])[1]
        except TypeError:
    ...
    def joueur_proche_ball_all(self, id_team, id_player):
        ...
        try:
            a=min([(self.ball.distance(player_e.position), player_e) for player_e in opponents])[1]
        except TypeError:
            a=self.state.player_state(self.det_team_e(id_team),0)
        return a
    ...
    def element_entre(self, debut, fin, obst):
        vdebut_fin = debut - fin
        vdebut_fin = obst - debut
    ...
    def joueurs_entre(self, debut, fin):
        ...
        for i in joueurs:
            ...
            if self.element_entre(debut, fin, obst):
                return True
        return False
    ...
    def proche_balle(self):
        ...
        for i in joueurs:
            if(i.position.distance(self.state.ball.position) < BALL_RADIUS + PLAYER_RADIUS):
                if self.joueur_proche_ball_all != self.joueur_proche_ball_a:
                    return 0 #ennemie le plus proche qui va frapper 
        
        if self.state.player_state(self.id_team, self.id_player).position.distance(self.ball) < BALL_RADIUS + PLAYER_RADIUS:
            return 1#moi qui vais frapper
    ...
    def shoot_safe(self):
        for i in range(0, GAME_HEIGHT):
            ...
            if self.joueurs_entre(self.ball, cible) == False:
                return cible
