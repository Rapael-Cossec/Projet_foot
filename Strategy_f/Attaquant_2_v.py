...
#        self.vitesse = vitesse
    def compute_strategy(self, state, id_team, id_player):
        ...
        if(id_team == 1):
            if(joueur_proche == s.joueur_proche_ball_a(id_team, id_player) or s.ball.y < 40):
                return SoccerAction(Vector2D(GAME_WIDTH*7/16, s.ball.y) - s.player, Vector2D(0, 0))
        else:
            if(joueur_proche == s.joueur_proche_ball_a(id_team, id_player)or s.ball.y > GAME_WIDTH - 40):
