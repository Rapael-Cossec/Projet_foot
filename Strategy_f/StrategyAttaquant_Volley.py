...
#        self.vitesse = vitesse
    def compute_strategy(self, state, id_team, id_player):
        ...
        if(id_team == 1):
            ...
            return SoccerAction(Vector2D(GAME_WIDTH/4, GAME_HEIGHT/2) - s.player, Vector2D(0, 0))
        else:
            if(s.ball.x > GAME_WIDTH/2):
                if(s.dir_ball.norm < CAN_SHOOT):
                    if(s.ball.x >= GAME_WIDTH/2 + 20):
                ...
                return SoccerAction(s.dir_ball.normalize().scale(5.0), Vector2D(0, 0))
            return SoccerAction(Vector2D(GAME_WIDTH*3/4, GAME_HEIGHT/2)-s.player, Vector2D(0, 0))